...
The function _ADXL345_Initialize()_ configures various registers of the ADXL345 accelerometer to set it up for activity and inactivity detection. It checks the device ID to ensure proper communication with the accelerometer.
...
The code defines two interrupt handlers (_activity_isr_handler_ and_ inactivity_isr_handler_) that get triggered when the   _INT_1_ and _INT_2_ GPIO pins change their state. These handlers set flags (_activity_flag_ and _inactivity_flag_) to indicate that activity or inactivity has been detected.

_void IRAM_ATTR activity_isr_handler(void* arg):_
This function is an interrupt service routine (ISR) that handles the interrupt triggered when activity is detected by the ADXL345 accelerometer.
...
The ISR does not take any arguments _(void* arg)_ because it is a simple handler for the activity interrupt.

Inside the ISR, the following actions are performed:
_activity_flag = 1;:_ The variable activity_flag is set to 1, indicating that activity has been detected.
_inactivity_flag = 0;:_ The variable inactivity_flag is set to 0, indicating that inactivity is not detected.

_void IRAM_ATTR inactivity_isr_handler(void* arg):_
...
The _IRAM_ATTR_ attribute is used to place the ISR in the IRAM (Internal RAM) section, which improves the ISR's execution time and reduces latency.
The ISR does not take any arguments _(void* arg)_ because it is a simple handler for the inactivity interrupt.

Inside the ISR, the following actions are performed:
_inactivity_flag = 1;:_ The variable inactivity_flag is set to 1, indicating that inactivity has been detected.
_activity_flag = 0;:_ The variable activity_flag is set to 0, indicating that activity is not detected.

**GPIO Configuration:**
The function _ISR_GPIO()_ configures the GPIO pins (_INT_1_ and _INT_2_) as input pins with interrupt-on-any-edge functionality. It installs the interrupt service routine (ISR) handlers for both pins to detect activity and inactivity.
...
_Configuring INT_1 GPIO pin for activity detection:_
...
The code for configuring _INT_2 GPIO pin_ is identical to the configuration of_ INT_1_, except it uses the _INT_2_ macro to represent the GPIO pin number for inactivity detection.
...
_gpio_isr_handler_add(INT_1, activity_isr_handler, (void*)INT_1);:_ This line registers the activity_isr_handler function as the ISR for the _INT_1_ GPIO pin. The ISR will be called when the INT_1 GPIO pin triggers an interrupt. The _(void*)INT_1_ argument is used to pass the _INT_1_ macro value as a parameter to the ISR function (if needed).

_gpio_isr_handler_add(INT_2, inactivity_isr_handler, (void*)INT_2);:_ This line registers the inactivity_isr_handler function as the ISR for the _INT_2_ GPIO pin. The ISR will be called when the INT_2 GPIO pin triggers an interrupt. The _(void*)INT_2_ argument is used to pass the _INT_2_ macro value as a parameter to the ISR function (if needed).
...
Checks the interrupt flags to print messages when activity or inactivity is detected.
